...
PATH_TO_IMAGES = '/home/ml/saliency-salgan-2017/images/origin'
PATH_TO_SALMAPS = '/home/ml/saliency-salgan-2017/images/'


def test(path_to_images, path_output_maps, model_to_test=None):
    list_img_files = [k.split('/')[-1].split('.')[0] for k in
                      glob.glob(os.path.join(path_to_images, '*'))]
    # Load Data
    list_img_files.sort()
    for curr_file in tqdm(list_img_files, ncols=20):
        print os.path.join(path_to_images, curr_file + '.jpg')
        img = cv2.cvtColor(cv2.imread(os.path.join(path_to_images, curr_file +
                            '.jpg'), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
        predict(model=model_to_test, image_stimuli=img, name=curr_file,
                path_output_maps=path_output_maps)
...
def main():
    ...
    load_weights(model.net['output'], path=model_name + '/gen_',
                 epochtoload=epochtoload)
    test(path_to_images=PATH_TO_IMAGES, path_output_maps=PATH_TO_SALMAPS +
        '/', model_to_test=model)
